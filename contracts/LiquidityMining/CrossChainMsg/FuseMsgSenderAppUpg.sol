...
abstract contract FuseMsgSenderAppUpg is BoringOwnableUpgradeable {
    using EnumerableMap for EnumerableMap.UintToAddressMap;

    uint256 public approxDstExecutionGas;

    IPMsgSendEndpoint public immutable fuseMsgSendEndpoint;
    ...
    constructor(address _fuseMsgSendEndpoint, uint256 _approxDstExecutionGas) {
        fuseMsgSendEndpoint = IPMsgSendEndpoint(_fuseMsgSendEndpoint);
    ...
    function _sendMessage(uint256 chainId, bytes memory message) internal {
        ...
        uint256 fee = fuseMsgSendEndpoint.calcFee(toAddr, chainId, message, estimatedGasAmount);
        ...
        fuseMsgSendEndpoint.sendMessage{value: fee}(toAddr, chainId, message, estimatedGasAmount);
    }
    ...
    function _getSendMessageFee(uint256 chainId, bytes memory message) internal view returns (uint256) {
        return
            fuseMsgSendEndpoint.calcFee(destinationContracts.get(chainId), chainId, message, approxDstExecutionGas);
    }
}
